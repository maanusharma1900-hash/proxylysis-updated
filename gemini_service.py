...
def identify_involved_glids(csl_logs, matchmaking_records, product_name):
    # If CSL logs are empty, use matchmaking data directly without calling Gemini
    if not csl_logs or len(csl_logs) == 0:
        print("CSL logs are empty, using matchmaking data directly for suspect identification")
        
        involved_glids = []
        seen_glids = set()
        
        for record in matchmaking_records or []:
            if record and isinstance(record, dict) and record.get("contacts_glid"):
                glid = record.get("contacts_glid")
                if glid not in seen_glids:
                    involved_glids.append({
                        "glId": glid,
                        "companyName": record.get("contacts_company", "Unknown Company"),
                        "lastProduct": record.get("contact_last_product", record.get("last_product_qty", "Unknown Product")),
                        "confidenceScore": "100%"  # Direct match from matchmaking data
                    })
                    seen_glids.add(glid)
        
        return involved_glids
